...
impl SGLangClient {
    ...
    pub async fn generate(&self, req: &InferenceRequest) -> Result<serde_json::Value, anyhow::Error> {
...
impl ConfidenceTracker {
    ...
    pub async fn log_weak(&self, req: &InferenceRequest, confidence: f32) {
        ...
        warn!("Logged weak confidence: {} for prompt: {}", confidence, req.prompt);
    }
}
    ...
    pub async fn route(&self, req: InferenceRequest) -> Result<serde_json::Value, anyhow::Error> {
        ...
        let local_result = match req.task_type {
            TaskType::Architecture | TaskType::Debugging => {
                self.local_think.generate(&req).await
            }
            TaskType::Syntax | TaskType::CodeCompletion => {
            ...
            Ok(resp) => {
                ...
                if confidence >= 0.6 {
                    ...
                    if req.local_failures >= 3 {
                        info!("Confidence too low ({}) and failures >= 3, calling cloud fallback", confidence);
