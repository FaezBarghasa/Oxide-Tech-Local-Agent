...
export const Header: React.FC<HeaderProps> = ({ currentTab, onSelectTab, onNewSession, onQuickDeploy }) => {
  ...
  return (
    <header className="h-12 border-b border-white/[0.05] flex items-center justify-between px-4 bg-[#0b0c10]/80 backdrop-blur-sm shrink-0">
      <div className="flex items-center gap-2.5 min-w-0">
        <span className="text-[10px] font-mono uppercase font-bold text-amber-400/70 px-1.5 py-0.5 rounded bg-amber-500/[0.06] border border-amber-500/[0.15]">
          {currentTab}
        </span>
        <div className="min-w-0">
          <h1 className="text-xs font-bold text-white tracking-tight truncate">{current.title}</h1>
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          className="px-3 py-1.5 rounded-lg bg-[#141418] hover:bg-[#1c1c22] text-zinc-300 border border-white/[0.08] hover:border-amber-500/[0.3] text-[11px] font-medium transition flex items-center gap-1.5 cursor-pointer"
        >
