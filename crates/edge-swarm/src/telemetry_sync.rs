...
impl EdgeSwarmBridge {
    ...
    pub async fn publish_telemetry(
        ...
        let payload = packet.to_bytes().map_err(|e| format!("Postcard encoding failed: {e:?}"))?;
        let mut mqttoptions = MqttOptions::new(&self.client_id, &self.broker_host, self.broker_port);
