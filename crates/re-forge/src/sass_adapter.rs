...
impl SassAdapter {
    ...
    pub async fn disassemble_cubin(
        &self,
        cubin_path: &Path,
    ) -> Result<SassDisassembly, SassAdapterError> {
        info!(
            "Disassembling CUDA binary {:?} via nvdisasm adapter",
            cubin_path
        );
        ...
        if !exists {
            warn!(
                "nvdisasm not found in PATH; returning labeled synthetic fallback SASS disassembly"
            );
            return Ok(SassDisassembly {
                ...
                instructions: vec![
                    SassInstruction {
                        ...
                        operands: "R0, [R2]".to_string(),
                    },
                    SassInstruction {
                        offset: 0x10,
                        opcode: "HMMA.16816.F32".to_string(),
                        operands: "R4, R0, R1, R4".to_string(),
                    },
                    SassInstruction {
                        offset: 0x20,
                        opcode: "STG.E".to_string(),
                        operands: "[R6], R4".to_string(),
                    },
                    SassInstruction {
                        offset: 0x30,
...
mod tests {
    ...
    async fn test_sass_adapter_fallback() {
        ...
        let res = adapter
            .disassemble_cubin(Path::new("dummy.cubin"))
            .await
            .unwrap();
