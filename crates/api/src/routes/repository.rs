...
pub async fn run_repository_structure(workspace_path: String) -> Result<FileNode, String> {
    ...
    let root_node = tokio::task::spawn_blocking(move || {
        ...
        let name = base_path.file_name().and_then(|n| n.to_str()).unwrap_or("root").to_string();
        FileNode {
            name,
            path: "".to_string(),
            is_dir: true,
    ...
    }).await.map_err(|e| format!("Tree building task panicked: {}", e))?;

    Ok(root_node)
}

#[get("/api/repository/structure")]
pub async fn handle_repository_structure(query: web::Query<RepoStructureRequest>) -> impl Responder {
