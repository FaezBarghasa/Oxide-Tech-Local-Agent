...
mod tests {
    ...
    async fn test_constitutional_membrane_file_whitelist() {
        ...
        // Allowed path
        let res_allowed = membrane
            .validate_action(1234, &SyscallCategory::FileWrite, "/tmp/sandbox_test.rs")
            .await;
        assert!(res_allowed.is_ok());

        // Forbidden path
        let res_denied = membrane
        ...
        assert!(
            violation
                .reason
                .contains("not in the constitutional whitelist")
        );
