...
use crate::ir::{
    UirField, UirFunction, UirItem, UirModule, UirParam, UirSelfKind, UirStmt, UirStruct, UirType,
};
        ...
        for cap in struct_re.captures_iter(source) {
            ...
            for line in body.lines() {
                ...
                if parts.len() >= 2 {
                    ...
                    let is_ptr =
                        parts[parts.len() - 1].starts_with('*') || field_type_str.ends_with('*');
            ...
            if !args_str.trim().is_empty() && args_str.trim() != "void" {
                for arg in args_str.split(',') {
                    ...
                    if parts.len() >= 2 {
                        ...
                        let is_ptr =
                            parts[parts.len() - 1].starts_with('*') || param_ty_str.ends_with('*');
                        ...
                        if param_name == "self" || param_name == "this" {
                            ...
                            struct_target =
                                Some(param_ty_str.trim_end_matches('*').trim().to_string());
                        } else {
...
fn map_c_type(ty: &str, is_ptr: bool) -> UirType {
    ...
    if is_ptr {
        match base {
            ...
            UirType::Void => UirType::RawPointer {
                mutable: true,
                inner: Box::new(UirType::U8),
            },
            other => UirType::Reference {
                mutable: true,
                inner: Box::new(other),
            },
        }
